module.exports = function solveSudoku(matrix) {
  ...
  function setAAA() {
      ...
      if(solve()==matrix){return nullArray;}
      for (var i = 0; i < 9; i++) {

          for (var j = 0; j < 9; j++) {

              html += solved[i][j][1] + '"' + ('unknown' == solved[i][j][1] ? +solved[i][j][2].join(', ') + '"' : '') + (solved[i][j][0] ? solved[i][j][0] : '&nbsp;');
          }
          arr[k] = parseInt(html.replace(/[in"solved]/g, ''));
          html = 0;
          k++;
      }
